...
stela_dir = os.path.dirname(os.path.realpath(__file__))
version_file = os.path.join(stela_dir, 'VERSION')
...
def construct_version_info():
    ...
    try:
        git_origin = subprocess.check_output(['git', '-C', stela_dir, 'config',
                                              '--get', 'remote.origin.url'],
                                             stderr=subprocess.STDOUT).strip()
        git_hash = subprocess.check_output(['git', '-C', stela_dir, 'rev-parse', 'HEAD'],
                                           stderr=subprocess.STDOUT).strip()
        git_description = subprocess.check_output(['git', '-C', stela_dir,
                                                   'describe', '--dirty', '--tag', '--always']).strip()
        git_branch = subprocess.check_output(['git', '-C', stela_dir, 'rev-parse',
                                              '--abbrev-ref', 'HEAD'],
                                             stderr=subprocess.STDOUT).strip()
        git_version = subprocess.check_output(['git', '-C', stela_dir, 'describe', '--always',
                                               '--tags', '--abbrev=0']).strip()
    except:  # pragma: no cover  - can't figure out how to test exception.
        try:
            # Check if a GIT_INFO file was created when installing package
            git_file = os.path.join(stela_dir, 'GIT_INFO')
...
stela_version_str = ('stela version: ' + version + '.')
if git_hash is not '':
    stela_version_str += ('  Git origin: ' + str(git_origin) +
